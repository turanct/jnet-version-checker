...
for file in `locate viewforum.php`
do
	...
	# Get version number
	InstalledVersion=`mysql -u $dbuser -D $dbname -p$dbpass -h $host --skip-column-names -e "select config_value from $dbprefix config where config_name='version';" | grep '[\d\.]*'`
